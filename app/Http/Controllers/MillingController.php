...
class MillingController extends Controller
{
    ...
    public function newmillingCoffee(Request $request)
    {
        ...
        foreach ($batches as $batch) {
            $transaction = Transaction::where('batch_number', $batch)->whereIn('sent_to', $senttoArr)->where('is_parent', 0)->with('details')->orderBy('transaction_id', 'desc')->first();
    ...
    public function newmillingCoffeeSearch(Request $request)
    {
        $transactions = collect();
        $batches = BatchNumber::pluck('batch_number');
        $senttoArr[0] = $request->sent_to;

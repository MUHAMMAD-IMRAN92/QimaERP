...
class User extends Authenticatable
{
    ...
    {
        $userId = $this->user_id;
        $transactions = Transaction::with('details')->where(['created_by' =>   $userId, 'is_special' => 2])->where('batch_number', 'NOT LIKE', '%000%')->where('sent_to', 2)->get();
        if ($transactions) {
            $totalWeight = 0;
            $totalPrice = 0;
            foreach ($transactions as $transaction) {
    ...
    {
        $userId = $this->user_id;
        $transactions = Transaction::with('details')->where(['created_by' =>   $userId, 'is_special' => 2])->where('sent_to', 3)->get();
        if ($transactions) {
            $totalWeight = 0;
            $totalPrice = 0;
            foreach ($transactions as $transaction) {
    ...
    {
        $userId = $this->user_id;
        $transactions = Transaction::with('details')->where(['created_by' =>   $userId])->where('batch_number', 'NOT LIKE', '%000%')->where('sent_to', $sent_to)->whereDate('created_at', $date)->get();
        if ($transactions) {
            $totalWeight = 0;
            $totalPrice = 0;
            foreach ($transactions as $transaction) {
    ...
    {
        $userId = $this->user_id;
        $transactions = Transaction::with('details')->where(['created_by' =>   $userId, 'is_special' => 2])->where('batch_number', 'NOT LIKE', '%000%')->where('sent_to', $sent_to)->whereDate('created_at', $date)->get();
        if ($transactions) {
            $totalWeight = 0;
            $totalPrice = 0;
            foreach ($transactions as $transaction) {
    ...
    {
        $userId = $this->user_id;
        $transactions = Transaction::with('details')->where(['created_by' =>   $userId, 'is_special' => 2])->whereMonth('created_at', $month)->whereYear('created_at',  $year)->where('batch_number', 'NOT LIKE', '%000%')->where('sent_to', $sent_to)->get();
        if ($transactions) {
            $totalWeight = 0;
            $totalPrice = 0;
            foreach ($transactions as $transaction) {
    ...
    {
        $userId = $this->user_id;
        $transactions = Transaction::with('details')->where(['created_by' =>   $userId])->whereMonth('created_at', $month)->whereYear('created_at',  $year)->where('batch_number', 'NOT LIKE', '%000%')->where('sent_to', $sent_to)->get();
        if ($transactions) {
            $totalWeight = 0;
            $totalPrice = 0;
            foreach ($transactions as $transaction) {
    ...
    {
        $userId = $this->user_id;
        $transactions = Transaction::with('details')->where(['created_by' =>   $userId])->whereYear('created_at', $year)->where('batch_number', 'NOT LIKE', '%000%')->where('sent_to', $sent_to)->get();
        if ($transactions) {
            $totalWeight = 0;
            $totalPrice = 0;
            foreach ($transactions as $transaction) {
    ...
    public function yearNonSpecialTransaction($year, $sent_to)
    {
        ...
        if ($sent_to == 2) {
            ...
            $transactions = Transaction::with('details')->where(['created_by' =>   $userId, 'is_special' => 2])->whereYear('created_at', $year)->where('batch_number', 'NOT LIKE', '%000%')->where('sent_to', $sent_to)->get();
        } else {
            $transactions = Transaction::with('details')->where(['created_by' =>   $userId, 'is_special' => 2])->whereYear('created_at', $year)->where('sent_to', $sent_to)->get();
        }
        if ($transactions) {
            $totalWeight = 0;
            $totalPrice = 0;
    ...
    public function betweenSpecialTransaction($start, $end, $sent_to)
    {
        ...
        if ($sent_to == 2) {
            ...
            $transactions = Transaction::with('details')->where(['created_by' =>   $userId])->whereBetween('created_at', [$start, $end])->where('batch_number', 'NOT LIKE', '%000%')->where('sent_to', $sent_to)->get();
        } else {

            $transactions = Transaction::with('details')->where(['created_by' =>   $userId])->whereBetween('created_at', [$start, $end])->where('sent_to', $sent_to)->get();
        }
        if ($transactions) {
            $totalWeight = 0;
            $totalPrice = 0;
    ...
    public function betweenNonSpecialTransaction($start, $end, $sent_to)
    {
        ...
        if ($sent_to == 2) {
            ...
            $transactions = Transaction::with('details')->where(['created_by' =>   $userId, 'is_special' => 2])->whereBetween('created_at', [$start, $end])->where('batch_number', 'NOT LIKE', '%000%')->where('sent_to', $sent_to)->get();
        } else {

            $transactions = Transaction::with('details')->where(['created_by' =>   $userId, 'is_special' => 2])->whereBetween('created_at', [$start, $end])->where('sent_to', $sent_to)->get();
        }
        if ($transactions) {
            $totalWeight = 0;
            $totalPrice = 0;
